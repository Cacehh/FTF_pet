jQuery(document).ready(function($){
		...
		this.steps.each(function(){
			...
			if( $(this).data('selection') == 'colors' ) {
				...
				var selectedOptions = $(this).find('.js-option.selected'),
					optionsContent = '';
				console.info("logging just to check if it passed here ");
				if( selectedOptions.length == 0 ) {
					optionsContent = '<li><p>No eye color selected</p></li>';
	...
	ProductBuilder.prototype.updateListOptions = function(listItem) {
		...
		var self = this;
		// added code
		//alert( listItem.siblings('.selected').length);
		
		if (listItem.siblings('.selected').length > 0){
